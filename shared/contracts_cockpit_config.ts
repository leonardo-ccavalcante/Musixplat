...
// The closed set of tier identities (= Restaurant.tier_base). A free-text tier_id would insert clean but
// resolve to no policy at dispatch (silently-dead config) — close it here so a typo is rejected, not silent.
export const TIER_BASES = ["managed_brand", "managed_midmarket", "long_tail"] as const;

export const knobUpsert = z.object({
  key: z.string().min(1), // must be an operator-tunable knob (allowlisted + range-checked server-side, fail-closed)
...
export const policyTierUpsert = z.object({
  ...
  tier_id: z.enum(TIER_BASES), // maps to Restaurant.tier_base (closed set)
