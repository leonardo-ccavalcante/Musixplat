...
-- Decision_Trace XOR origin: setting BOTH release_batch_id and episode_id is rejected.
...
insert into gov."Release_Batch"(release_batch_id, cohort_id, action, proposer_id, operator_id)
  values ('L-test', 'cohort_test', 'RELEASE', 'U-PROP', 'U-CONF');
select throws_ok(
  $$ insert into gov."Decision_Trace"(trace_id, release_batch_id, episode_id, action, proposer_id, policy_version)
     values ('t-xor', 'L-test', 'E-test', 'release', 'U-PROP', 'pv-test') $$,
  '23514', NULL, 'XOR origin rejects both release_batch_id and episode_id set');
