...
-- The motor CHECK rejects a hand-faked effective_level (must equal least of the 3 arms).
select throws_ok(
  ...
  '23514', NULL, 'motor CHECK rejects a effective_level that is not least(arms)');

-- Exactly one origin (nba_id XOR conversation_id).
select throws_ok(
  $$ insert into gov."min_calculation"(nba_id,conversation_id,nba_request,released_evals,tier_cap,effective_level)
