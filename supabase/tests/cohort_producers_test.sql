...
-- valor_neto is GENERATED arithmetic (bruto - fee), not a seeded result.
...
select is((select valor_neto from tenant."Orden" where restaurante_id='RT01'), 80.00::numeric, 'valor_neto = bruto - fee');

insert into catalog."Cohort_Rule_Version"(version_id, fecha, que_cambio) values ('vtest', date '2026-06-01', 'test');
...
select is((select supresion_k_aplicada from cohort."Cohort" where cohort_id='ck4'), true, 'k-anon: n<5 suppresses');
select is((select supresion_k_aplicada from cohort."Cohort" where cohort_id='ck5'), false, 'k-anon: n>=5 passes');
select is((select supresion_k_aplicada from cohort."Cohort" where cohort_id='cknull'), true, 'k-anon: NULL fail-closed');
...
select is((select colapsada from cohort."Cohort" where cohort_id='cn19'), true, 'n_min: n<20 collapses');
select is((select colapsada from cohort."Cohort" where cohort_id='cn20'), false, 'n_min: n>=20 ok');
