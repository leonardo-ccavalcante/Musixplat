...
-- Config_Perillas: every threshold read BY NAME (k_anon_threshold, n_min_threshold,
-- tenure borders, TTL_baseline, D, ...). CLAUDE.md §3.8 — never a hard-coded literal.
create table catalog."Config_Perillas" (
...
create or replace function catalog.perilla_num(p_key text)
returns numeric language sql stable as $$
  select case
    when not exists (select 1 from catalog."Config_Perillas" where key = p_key)
      then (select null::numeric where false)
    else (select valor::numeric from catalog."Config_Perillas" where key = p_key)
  end;
$$;

create or replace function catalog.perilla_required_num(p_key text)
returns numeric language plpgsql stable as $$
declare v numeric;
begin
  select valor::numeric into v from catalog."Config_Perillas" where key = p_key;
  if v is null then
    raise exception 'Config_Perillas missing required knob: %', p_key;  -- fail-closed
  end if;
  return v;
end;
$$;

create or replace function catalog.perilla_text(p_key text)
returns text language plpgsql stable as $$
declare v text;
begin
  select valor into v from catalog."Config_Perillas" where key = p_key;
  if v is null then
    raise exception 'Config_Perillas missing required knob: %', p_key;  -- fail-closed
  end if;
  return v;
