...
-- Conversation_Episode with the columns piece A.1.1 needs (channel, turns, structured_layer,
-- provenance_by_field) and creates the gov-zone min() motor log (min_calculation) that piece
-- A.4.6 writes. 04 §3.1 / §3.3 / §14. DDL only — raw inputs/config live in seed; min_calculation is
...
alter table tenant."Conversation_Episode"
  ...
  add column turns                jsonb not null default '[]'::jsonb,    -- bruto sub-records, no child table (A.1.1)
  add column structured_layer     jsonb not null default '{}'::jsonb,    -- causa_hipotesis [I]/policy_version/... (A.2.6/A.6.3)
  add column provenance_by_field jsonb not null default '{}'::jsonb;    -- per-field provenance (04 §7)

-- min_calculation (04 §3.3): append-only motor log. effective_level = least(3 arms) over the
...
create table gov."min_calculation" (
  ...
  constraint min_calculation_motor check (effective_level = least(nba_request, released_evals, tier_cap))
);
