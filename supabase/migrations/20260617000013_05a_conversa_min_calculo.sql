...
-- provenance_by_field) and creates the gov-zone min() motor log (min_calculation) that piece
...
-- Support-conversationtion channel (04 §3 Conversation_Episode). Closed label set. Distinct from
...
-- min_calculation (04 §3.3): append-only motor log. level_efectivo = least(3 arms) over the
...
create table gov."min_calculation" (
  ...
  pedido_NBA          public.autonomy_level not null,
  liberado_evals      public.autonomy_level not null,
  teto_tier           public.autonomy_level not null,
  level_efectivo      public.autonomy_level not null,       -- RESULT: computed at insert, never seeded (§14)
  auto_liberable      boolean default null,                  -- RESULT §14
  n_cohort            integer default null,
  cohort_rule_version text,
  computed_at         timestamptz not null default now(),
  constraint min_calculation_origen_xor check ((nba_id is not null) <> (conversation_id is not null)),
  constraint min_calculation_motor check (level_efectivo = least(pedido_NBA, liberado_evals, teto_tier))
);
...
create or replace function gov.compute_level_efectivo(
