...
-- entities (Eval_Cell, NBA_Proposal, Policy_Tier, Credential, Release_Batch,
...
--
-- Verbatim sources (quoted in the build report): NBA_Proposal 04 L221 · Eval_Cell 04 L212 ·
-- Policy_Tier 04 L246 · Credential 04 L255 · Release_Batch 04 L262 · Decision_Trace
-- 04 L269-270 · Conversation_Episode 04 L107/L112. NBA_Proposal is NOT on the §3.9 phantom
...
--    min_calculation / Conversation_Episode can FK it; seed inserts NO rows (§14). realized_impact
--    is a RESULT (NULL pre-run · §14 L684); expected_before_after / estimated_impact are
...
create table gov."NBA_Proposal" (
  ...
  expected_before_after jsonb default null,                               -- projection-NOT-measured [C] (§14 L688)
  ...
  estimated_impact      jsonb default null,                               -- projection-NOT-measured [C] (§14 L688)
  realized_impact       jsonb default null,                               -- RESULT §14 L684 (only from real signal, never copied from estimated · BR-HON-1)
...
--    redteam_result) are RESULT: run the golden-set, never type "green"/"LOW" (§14 L681).
...
create table gov."Eval_Cell" (
  ...
  redteam_result_judge_vs_human   text                  default null,    -- RESULT §14 L681
  provenance_by_field            jsonb not null default '{}'::jsonb,     -- per-field provenance (04 §7)
...
--    requiere_2_ojos, origen_permitido). level_max_liberable never exceeds tier_cap (enforced
...
create table gov."Credential" (
  ...
  rbac_matrix          jsonb not null default '{}'::jsonb,                 -- RBAC matrix (role×accion_class → level_max_liberable/requiere_2_ojos/origen_permitido)
...
create table gov."Decision_Trace" (
  ...
  time_to_sign_seconds     integer default null,                          -- RESULT §14 L686
  -- [ASSUMPTION: rubber_stamp_flag is a RESULT column DEFAULT NULL, NOT a STORED GENERATED column.
  --  04 L270 says "(generada) = (time_to_sign_seconds < threshold AND origin='mobile')" but the threshold
  ...
  constraint decision_trace_4ojos check (confirmer_id is null or confirmer_id <> proposer_id),  -- 4-eyes (04 L270)
  constraint decision_trace_origin_xor check ((release_batch_id is not null) <> (episode_id is not null))  -- XOR origin (governance vs conversation)
);
create index decision_trace_release_batch_idx on gov."Decision_Trace"(release_batch_id);
...
create table gov."Release_Batch" (
  ...
  validated_policy_version text references gov."Policy_Tier"(policy_version),
  ...
  created_at               timestamptz not null default now(),
  constraint release_batch_4ojos check (proposer_id <> operator_id)   -- 4-eyes: proposer != signer (task constraint)
);
...
alter table gov."Decision_Trace"
  add constraint decision_trace_release_batch_fk
  foreign key (release_batch_id) references gov."Release_Batch"(release_batch_id);
