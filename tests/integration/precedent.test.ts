...
async function seedCase(status: string, actionCode: string): Promise<void> {
  const [vec] = await deterministicEmbedder.embed(["m_connection_below"]); // same text the query embeds ⇒ kNN match
  await pool.query(
...
describe("05D Part B — acceptPrecedent (precedent-first accept-gate)", () => {
  ...
  it("ACCEPTS a verified_fixed precedent whose lever re-confirms — acts on the CURRENT verdict", async () => {
    ...
    const out = await acceptPrecedent(TENANT, confirming, "m_connection_below", exec, deterministicEmbedder);
  ...
  it("REJECTS an unverified precedent (kNN only retrieves verified_fixed — never a raw 'acted')", async () => {
    ...
    expect(await acceptPrecedent(TENANT, confirming, "m_connection_below", exec, deterministicEmbedder)).toBeNull();
  });
  ...
  it("REJECTS a verified precedent whose signal NO LONGER re-confirms (verdict is ok now)", async () => {
    ...
    expect(await acceptPrecedent(TENANT, resolved, "m_connection_below", exec, deterministicEmbedder)).toBeNull();
  });
  ...
  it("tenant-scoped: a verified precedent in another pool is never retrieved (§3.4)", async () => {
    await seedCase("verified_fixed", "act_reset");
    expect(await acceptPrecedent("POOL-OTHER", confirming, "m_connection_below", exec, deterministicEmbedder)).toBeNull();
  });
});
