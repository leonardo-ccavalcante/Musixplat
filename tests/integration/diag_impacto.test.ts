...
// (tenant.fn_impact_lost_revenue); TS orchestrates. Anti-fake §14: lost_revenue / cost_to_resolve /
...
// expected lost_revenue = 80 + 40 + 150 = 270 (the ok order is excluded by payment_status).
...
async function seedFixture(): Promise<string> {
  ...
  // Affected set = both fixture restaurants (hunt-silent output, stubbed here directly).
  await pool.query(
...
describe("05B:US-B5.1.1 — computeRsPerdido (Named_Query, churn fail-closed NULL)", () => {
  it("anti-fake §14: lost_revenue is NULL before computeRsPerdido runs", async () => {
    const problemaId = await seedFixture();
    const before = await rows<{ lost_revenue: string | null; churn_risk: string | null }>(
      pool,
      `select lost_revenue, churn_risk from tenant."Diagnosed_Problem" where problem_id = $1`,
      [problemaId],
    );
    expect(before[0]?.lost_revenue).toBeNull();
    ...
    const after = await rows<{ lost_revenue: string | null; churn_risk: string | null; prov: Record<string, string> }>(
      pool,
      `select lost_revenue, churn_risk, provenance_by_field as prov
         from tenant."Diagnosed_Problem" where problem_id = $1`,
      [problemaId],
    );
    expect(Number(after[0]?.lost_revenue)).toBe(EXPECTED_RS_PERDIDO);
    expect(after[0]?.churn_risk).toBeNull(); // stays NULL in the DB (fail-closed).
    expect(after[0]?.prov.lost_revenue).toBe("[I]"); // worst-provenance stamp written by the fn.
  });
});
