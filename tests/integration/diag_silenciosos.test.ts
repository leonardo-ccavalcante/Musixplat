...
// no observable population ⇒ not_evaluable, NEVER assume zero silent ones. tenant scoping = BR-B6.
//
...
const N_AFFECTED = 4;
const M_COMPLAINANTS = 2;
const WINDOW = 30; // matches the seeded 'window_silent' [C] knob; passed explicitly for determinism.
...
describe("05B:B.5.2b — huntSilent (anti-join in SQL, counts equal, §14 no seed)", () => {
  it("anti-fake §14: Affected is EMPTY before huntSilent runs", async () => {
    const problemId = await seedFixture();
    // No producer ran yet ⇒ zero Affected rows for this problem (rows are NEVER seeded).
    expect(await count(pool, `tenant."Affected" where problem_id='${problemId}'`)).toBe(0);

    const out = await huntSilent(problemId, "POOL-DIAG", WINDOW);

    // affected = every restaurant with a failed Order in window (the ok order adds none).
    expect(out.affected).toBe(N_AFFECTED);
    // silent = affected WITHOUT a Conversation = N − M (the ⭐ silent-hunter count).
    expect(out.silent).toBe(N_AFFECTED - M_COMPLAINANTS);
    ...
    expect(af).toHaveLength(N_AFFECTED);
  ...
  it("idempotent re-run: counts stay equal (ON CONFLICT DO NOTHING, no double-count)", async () => {
    const problemId = await seedFixture();
    await huntSilent(problemId, "POOL-DIAG", WINDOW);
    const again = await huntSilent(problemId, "POOL-DIAG", WINDOW);
    expect(again.affected).toBe(N_AFFECTED); // count(*) is truth, not the inserted-row return.
    expect(again.silent).toBe(N_AFFECTED - M_COMPLAINANTS);
  });

  it("window from the knob when windowDays is omitted (BY NAME, §3.8)", async () => {
    const problemId = await seedFixture();
    const out = await huntSilent(problemId, "POOL-DIAG"); // no explicit window ⇒ reads knob.
    expect(out.affected).toBe(N_AFFECTED);
    expect(out.silent).toBe(N_AFFECTED - M_COMPLAINANTS);
  });
  ...
  it("BR-B6 tenant scoping: a different tenant sees ZERO of POOL-DIAG's affected", async () => {
    const problemId = await seedFixture();
    // Same problem, wrong tenant ⇒ the anti-join is scoped by p_tenant ⇒ no rows match.
    const out = await huntSilent(problemId, "POOL-OTHER", WINDOW);
    expect(out.affected).toBe(0);
    expect(out.silent).toBe(0);
  });
});

describe("05B:US-B1.3.1 — reconcileAffected (live count + not_evaluable fail-closed)", () => {
  it("evaluable: restaurantsAffected = count(Affected), flag persisted", async () => {
    const problemId = await seedFixture();
    await huntSilent(problemId, "POOL-DIAG", WINDOW);

    const out = await reconcileAffected(problemId);
    expect(out.restaurantsAffected).toBe(N_AFFECTED); // live count, NEVER a stored number.
    expect(out.silentStatus).toBe("evaluable"); // population (Order in window) exists.
  ...
  it("BR-B4 fail-closed: no population in window ⇒ not_evaluable (never assume zero)", async () => {
    ...
    const out = await reconcileAffected(problemId);
    expect(out.restaurantsAffected).toBe(0);
    expect(out.silentStatus).toBe("not_evaluable"); // fail-closed: zero is NOT asserted as fact.
