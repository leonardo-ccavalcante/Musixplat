...
// (Order fallido ∖ Conversation-reclamantes) lives in SQL (tenant.fn_cazar_silenciosos); TS only
// orchestrates (§3.6). Anti-fake §14: Affected is EMPTY pre-run — its rows are produced by the
// fn, never seeded. ventana via the 'window_silent' knob BY NAME (§3.8). BR-B4 fail-closed:
...
// set, isolated from the seed's POOL-001 per-restaurant fallido noise. The spec's "47/35" is an
// illustrative scenario placeholder [C]; this proves the LOGIC with a small deterministic set.
//   N = 4 afetados   (each restaurant has ≥1 fallido Order inside the window)
//   M = 2 reclamantes (have a Conversation_Episode in the tenant ⇒ reclamou=true)
...
//   R-SIL-1: fallido + Conversation  → afetado, reclamou
//   R-SIL-2: fallido + Conversation  → afetado, reclamou
//   R-SIL-3: fallido, NO Conversation → afetado, SILENCIOSO
//   R-SIL-4: fallido + ok,  NO Conversation → afetado (only fallido drives it), SILENCIOSO
...
describe("05B:B.5.2b — cazarSilenciosos (anti-join in SQL, counts equal, §14 no seed)", () => {
  it("anti-fake §14: Affected is EMPTY before cazarSilenciosos runs", async () => {
    ...
    expect(await count(pool, `tenant."Affected" where problema_id='${problemaId}'`)).toBe(0);

    const out = await cazarSilenciosos(problemaId, "POOL-DIAG", VENTANA);

    // afetados = every restaurant with a fallido Order in window (the ok order adds none).
    expect(out.afetados).toBe(N_AFETADOS);
    // silenciosos = afetados WITHOUT a Conversation = N − M (the ⭐ caza-silenciosos count).
    ...
    const af = await rows<{ restaurant_id: string; reclamou: boolean; silencioso: boolean }>(
      pool,
      `select restaurant_id, reclamou, silencioso from tenant."Affected"
         where problema_id = $1 order by restaurant_id`,
      [problemaId],
    );
    expect(af).toHaveLength(N_AFETADOS);
    // R-SIL-3 / R-SIL-4 are silencioso (no ticket); R-SIL-1 / R-SIL-2 reclamaram.
    expect(af.filter((a) => a.silencioso).map((a) => a.restaurant_id)).toEqual(["R-SIL-3", "R-SIL-4"]);
    expect(af.filter((a) => a.reclamou).map((a) => a.restaurant_id)).toEqual(["R-SIL-1", "R-SIL-2"]);
  });
...
describe("05B:US-B1.3.1 — reconcileAffected (live count + no_evaluable fail-closed)", () => {
  it("evaluable: restaurantsAffecteds = count(Affected), flag persisted", async () => {
    ...
    const persisted = await rows<{ silenciosos_status: string | null }>(
      pool,
      `select silenciosos_status from tenant."Diagnosed_Problem" where problema_id = $1`,
      [problemaId],
    );
    expect(persisted[0]?.silenciosos_status).toBe("evaluable");
  });
  ...
  it("BR-B4 fail-closed: no population in window ⇒ no_evaluable (never assume zero)", async () => {
    ...
    expect(persisted[0]?.silenciosos_status).toBe("no_evaluable");
  });
});
