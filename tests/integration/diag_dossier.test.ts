...
// emit + gaps; once all 11 sources are filled + an Afetado exists ⇒ emit with an 11-field dossier.
...
describe("05B:US-B6.3.1 — emitDossier completeness + provenance gate", () => {
  it("bare Problema (RESULT fields null) ⇒ emitted=false, gaps list the empty fields (fail-closed)", async () => {
    ...
    // f6_recurrencia is always present (count + primera_vez_ts default), so it is NOT a gap.
  ...
  it("all 11 sources filled + ≥1 Afetado ⇒ emitted=true with an 11-field dossier", async () => {
    const r = await rows<{ problema_id: string }>(
      pool,
      `select problema_id from tenant."Problema_Diagnosticado" where restaurante_id = 'R001'`,
    );
    ...
    await rows(
      pool,
      `update tenant."Problema_Diagnosticado" set
         tipo_area      = 'cobranca',
         ...
         caso_repo      = jsonb_build_object(
                            'onde_concentra', jsonb_build_object('zona','centro'),
                            'dados_crudos',   jsonb_build_object('ordenes_fallidas', 5)),
         ruta_sugerida  = 'escalar a soporte de pagos',
         links_similares      = '[{"kb_case_id":"kb-1"}]'::jsonb,
         provenance_por_campo = jsonb_build_object('rs_perdido','[I]','tipo_area','[I]')
       where problema_id = $1`,
      [problemaId],
    );
    // f3_quien: at least one Afetado (caza-silenciosos output). evidencia is nullable.
    await rows(
      pool,
