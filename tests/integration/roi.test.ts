...
// Gate 5 — 1:10 producer + health read. ratio_1_10 = units(AI-processed) / human_touches, DERIVED from
// PRODUCED counts (§14): NULL pre-run, NULL until a human touches (fail-closed), and it FALLS as touches
// (escalations + artifact reviews) rise. health.summary READS it (vitrina; never recomputes the business
// number). is_attributable stays false (efficiency is provisional, not 2-gate confirmed — no green-fake).
...
describe("Gate 5 — 1:10 producer (ratio_1_10 DERIVED) + health.summary (vitrina)", () => {
  ...
  it("approving an artifact PRODUCES a ratio; a further escalation makes it FALL", async () => {
    ...
    expect(h1.ratio).toBe(3); // units 3 / touches 1
    ...
    expect(h2.ratio).not.toBeNull();
    expect(h2.ratio!).toBeLessThan(h1.ratio!); // 3/2 < 3/1 — escalation up ⇒ ratio down
  });
