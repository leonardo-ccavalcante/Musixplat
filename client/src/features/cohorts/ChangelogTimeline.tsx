...
export type RuleVersion = {
  ...
  fecha: string;
  que_cambio: string;
  efecto_en_baseline: string | null;
  provenance: string;
};

// F-4.2 — ML changelog timeline (read-only, ordered by fecha). Semantic list, navigable.
          ...
          {versions.map((v) => (
            <li key={v.version_id} className="border-l-2 border-mxm-border pl-3">
              <div className="flex items-center gap-2">
                ...
                <span className="tabnum text-xs text-mxm-content-tertiary">{v.fecha}</span>
                <span className="text-xs text-mxm-content-tertiary">{v.provenance}</span>
              </div>
              <p className="text-xs text-mxm-content-secondary">{v.que_cambio}</p>
              {v.efecto_en_baseline && (
                <p className="text-xs text-mxm-content-tertiary">→ {v.efecto_en_baseline}</p>
              )}
            </li>
          ))}
        </ol>
