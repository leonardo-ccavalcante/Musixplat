...
// BR-B2: never sweeps all sources (Occam → anti-hallucination + performance).
...
export function resolveSource(hypothesis: string): Source {
  ...
  return "tenant.Conversation_Episode"; // default — never sweeps all sources (BR-B2).
}
...
/**
 * US-B2.3.1 — lazy fetch. Resolves and consults ONLY the active path's single
 * source; every other path in the tree is returned untouched (source_consulted stays null).
