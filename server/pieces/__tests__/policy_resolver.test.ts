...
describe("resolvePolicy — 05A:A.2.3", () => {
  it("one matching tier + current version ⇒ sealed=true, reason=ok", () => {
    const result = resolvePolicy([baseRow], "tier_gold", CURRENT);
    expect(result.sealed).toBe(true);
    expect(result.reason).toBe("ok");
    expect(result.policy_version).toBe("v2");
    expect(result.tierCap).toBe("HIGH");
    expect(result.allowedToday).toEqual({ max_discount: 30 });
  });

  it("candidates exist but ALL stale version ⇒ sealed=false, reason=stale", () => {
    const staleRow: PolicyRow = { ...baseRow, policy_version: "v1" };
    const result = resolvePolicy([staleRow], "tier_gold", CURRENT);
    expect(result.sealed).toBe(false);
    expect(result.reason).toBe("stale");
    expect(result.tierCap).toBeNull();
    expect(result.allowedToday).toBeNull();
  });

  it("no candidate for tier ⇒ sealed=false, reason=none", () => {
    const result = resolvePolicy([baseRow], "tier_silver", CURRENT);
    expect(result.sealed).toBe(false);
  ...
  it("two candidates same tier + same version ⇒ sealed=false, reason=ambiguous", () => {
    ...
    const result = resolvePolicy([baseRow, row2], "tier_gold", CURRENT);
    expect(result.sealed).toBe(false);
    expect(result.reason).toBe("ambiguous");
  });

  it("null candidates ⇒ sealed=false, reason=none (fail-closed §3.7)", () => {
    const result = resolvePolicy(null, "tier_gold", CURRENT);
    expect(result.sealed).toBe(false);
    expect(result.reason).toBe("none");
  });

  it("undefined candidates ⇒ sealed=false, reason=none (fail-closed §3.7)", () => {
    const result = resolvePolicy(undefined, "tier_gold", CURRENT);
    expect(result.sealed).toBe(false);
    expect(result.reason).toBe("none");
  });

  it("empty array ⇒ sealed=false, reason=none", () => {
    const result = resolvePolicy([], "tier_gold", CURRENT);
    expect(result.sealed).toBe(false);
    expect(result.reason).toBe("none");
  });
  ...
  it("mix of stale + current rows for tier ⇒ sealed=true (current wins, not ambiguous)", () => {
    ...
    const result = resolvePolicy([staleRow, baseRow], "tier_gold", CURRENT);
    expect(result.sealed).toBe(true);
    expect(result.reason).toBe("ok");
    expect(result.policy_version).toBe("v2");
  });
  ...
  it("deterministic: same inputs produce same output", () => {
    const r1 = resolvePolicy([baseRow], "tier_gold", CURRENT);
    const r2 = resolvePolicy([baseRow], "tier_gold", CURRENT);
