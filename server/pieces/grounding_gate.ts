 ...
 * @param ttlMs  - TTL threshold in ms; supplied by caller from Config_Perillas TTL_baseline_days.
 */
export function hardGroundingGate(
  ...
  if (checks == null) {
    return { pass: false, eje: "grounding", estado: "no_verificable" };
  }

  const result = groundingGate(checks, ttlMs);

  if (result.verified) {
    return { pass: true, eje: null, estado: result.estado };
  }

  return { pass: false, eje: "grounding", estado: result.estado };
}
