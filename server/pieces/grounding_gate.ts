...
// Fail-closed: any failure ⇒ pass=false + axis='grounding' (§3.7). Deterministic, no LLM.
// (04 §7)
 ...
 * Any failure (including null/undefined input) ⇒ pass=false, axis='grounding' (fail-closed §3.7).
...
export function hardGroundingGate(
  ...
  if (checks == null) {
    return { pass: false, axis: "grounding", status: "no_verificable" };
  }

  const result = groundingGate(checks, ttlMs);

  if (result.verified) {
    return { pass: true, axis: null, status: result.status };
  }

  return { pass: false, axis: "grounding", status: result.status };
}
