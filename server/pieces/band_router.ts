...
/** Fallback Level for any invalid/missing input — most conservative (escalation). */
 ...
 */
export function routeBand(effectiveLevel: Level | null | undefined | string): BandRoute {
  const level: Level = VALID_LEVELS.has(effectiveLevel as string)
    ? (effectiveLevel as Level)
    : FALLBACK;

  const route: "A.5" | "A.7" = level === "LOW" ? "A.5" : "A.7";

  return { route, effectiveLevel: level };
}
